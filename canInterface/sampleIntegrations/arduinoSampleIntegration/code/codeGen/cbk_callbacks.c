...
/**
 * Register all frames of cluster arduinoSampleIntegration at the interface engine, which are
 * either sent or received by the implemented network node PowerDisplay.
 */
void cbk_initInterface()
{
    unsigned int idxMto = 0;
    unsigned int idxEdeEvSrc EDE_DBG_ONLY;
    ...
    const unsigned int noRegisteredRxMsgs EDE_DBG_ONLY = idxMto;
