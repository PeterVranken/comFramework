...
static void onReceiveFrame_PT_1024(const ede_callbackContext_t *pContext)
{
    ...
    bool reinitSQCValidation = (pck_API_PT_StateEcu01_1024.stsTransmission & ~cbk_stsTransm_errSequence) != 0;
...
static void onReceiveFrame_PT_1040(const ede_callbackContext_t *pContext)
{
    ...
    /* Unwanted sequence error artefacts will always be seen after another error, which
       inibited the decoding of the predecessors's sequence counter value. If we have these
       errors, we need to reinitialize the sequence error validation. */
    bool reinitSQCValidation = (pck_API_PT_StateEcu02_1040.stsTransmission & ~cbk_stsTransm_errSequence) != 0;
    ...
    ede_retriggerSingleShotTimer( pContext
                                ...
                                , RELATIVE_TIMEOUT_FRAME_RECEPTION * 50 /* ms */
                                );
...
static void onReceiveFrame_PT_2032(const ede_callbackContext_t *pContext)
{
    ...
    /* Unwanted sequence error artefacts will always be seen after another error, which
       inibited the decoding of the predecessors's sequence counter value. If we have these
       errors, we need to reinitialize the sequence error validation. */
    bool reinitSQCValidation = (pck_API_PT_UserLimits_2032.stsTransmission & ~cbk_stsTransm_errSequence) != 0;
...
static void onTimeoutFrame_PT_1024(const ede_callbackContext_t *pContext)
{
...
#if defined(DEBUG) &&  EDE_ENABLE_TIMER_CONTEXT_DATA == 1
    ...
    const uint8_t * const pEvContents = ede_getEventData(pContext, &sizeOfEvContentData);
    assert(sizeOfEvContentData == sizeof(uintptr_t));
    const uintptr_t evContentWord = *(const uintptr_t*)pEvContents;
    assert(evContentWord == 8);
#endif
...
static void onSendFrame_PT_1536(const ede_callbackContext_t *pContext)
{
    ...
    /* Send the frame. */
    if(oss_sendCanFrame(ede_getEventSourceData(pContext), frameContents, /* DLC */ 6))
    {
...
static void onSendFrame_PT_1538(const ede_callbackContext_t *pContext)
{
    ...
    /* Send the frame. */
    if(oss_sendCanFrame(ede_getEventSourceData(pContext), frameContents, /* DLC */ 1))
    {
        /* Bit operation not required here, there's only one error bit for outbound frames. */
...
static void onInitFrame_PT_1040(const ede_callbackContext_t *pContext)
{
    ...
    _hTimerTimeout_PT_1040 = ede_createSingleShotTimer
                                    ( pContext
                                    , RELATIVE_TIMEOUT_INITIAL_FRAME_RECEPTION * 50 /* ms */
...
void cbk_initInterface()
{
    ...
    ede_senderHandleEvent_t hFrame = OSS_INVALID_CAN_FRAME_HANDLE;
    unsigned int idxDispatcherSystem;
    ...
    /* Query the appropriate dispatcher to use for this frame in a second call
       of external, integrator provided code. */
    idxDispatcherSystem = mhn_assignFrameToDispatcher
                                    ( CEV_STD_CAN_ID(1024)
    ...
    /* Feed the sender map with the information, which dispatcher to use for an Rx event of
       this frame. */
    success = mhn_addMapEntrySenderEvHandleToSenderPortIndex
                                        ( /* hInstance */ 0u
                                        , /* kindOfEvent */ APT_EV_RX_CAN_MSG
                                        , /* senderHandleEvent */ hFrame
                                        , /* idxPort */ idxDispatcherSystem
                                        );
    assert(success);

    /* Having collected all required information the frame is now registered in the
       interface engine. */
    ede_registerExternalEventSource( apt_hDispatcherSystemAry[idxDispatcherSystem]
                                   , /* idxDispatcher */ 0u
                                   , /* kindOfEvent */ APT_EV_RX_CAN_MSG
                                   , /* senderHandleEvent */ hFrame
                                   , /* callback */ onInitFrame_PT_1024
                                   , /* refEventSourceData */ hFrame
                                   );
    ...
    /* Having collected all required information the frame is now registered in the
       interface engine. */
    ede_registerInternalEventSource( apt_hDispatcherSystemAry[idxDispatcherSystem]
                                   , /* idxDispatcher */ 0u
