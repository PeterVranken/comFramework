...
/** The cycle time in which the task timing statistics is printed to the console. THe unit
    ...
    acquisition.) */
#define CYCLE_TIME_TASK_STATISTICS_MS   2000
...
typedef struct threadData_t
{
    ...
    /** The thread is designed as a regularly executed task function. Here is the task
        function. The thread terminates if it returns \a rtos_taskFctResult_endOfScheduling. */
 ...
 * version and some implementations can hypothetically report a problem. These problems are
...
static void *thread(void *pThreadParamData)
{
    ...
    /* Run the task function and schedule the next invokation as long as it returns
         ...
         )
    {
        ...
        /* This is the awake point for the next cycle; take the time for the timing
           ...
           not a fault to display this shift as a thread timing inaccuracy - even if
           doesn't come from the pthreads library implementation. */
        ...
        if(--cntResync == 0)
        {
            ...
            /* tiNow should be "identical" to tiDue. If it deviates significantly we assign
               the current value to the locally updated one. The deviation is computed with
               low resolution; the nano second part of the time designations is not
               considered. The effectivly possible deviation without triggering a
               resynchronization is in the range (-MAX_DEVIATION_S-1, MAX_DEVIATION_S+1)
               seconds. */
            const signed long long tiDelta = tiDue.tv_sec - tiNow.tv_sec;
    ...
    /* If the task requested the end of scheduling then signal this to the parent thread.
         CAUTION: While the signalling through the volatile boolean is safe is the attempt
       ...
       other threads as soon as possible. In rare cases however it'll be in the middle of
 ...
 * application is reached. Then it returns once \a rtos_taskFctResult_endOfScheduling.
 */
...
{
    printf( "%s: Task timing data:\n", __func__);
    printf("-------------------------------------------------------------------\n");
    printf( "|%-22s|%-6s|%-9s|%-8s|%-7s|%-8s|\n"
           , "Name"
           , "#"
           , "mean [ms]"
           , "std [ms]"
    ...
    for(u=0; u<noThreads; ++u)
    {
        ...
        printf( "|%-22s|%-6u|%-9.1f|%-8.3f|%-7.3f|%-8lu|\n"
                      ...
                      )
{
    ...
    printf( "%s: Windows time quantum set to %u ms%s\n"
          , __func__
          , (unsigned)tiOsTick
...
#undef MAX_TI_CYCLE_TIMER_TASK
    ...
    {
        printf( "%s: Creating thread %u with cycle time %u ms\n"
              , __func__
              , u
        ...
        if(rc)
        {
            printf("%s: ERROR; return code from pthread_create() is %d\n", __func__, rc);
            fflush(stdout);
            break;
        }
    }
...
#undef MIN_TI_CYCLE_TIMER_TASK
    ...
    for(u=0; u<noRunningThreads; u++)
    {
        ...
        printf("%s: Waiting for termination of thread %u ...\n", __func__, u);
        fflush(stdout);
...
//        fflush(stdout);
    ...
    if(mmRes != TIMERR_NOERROR)
        printf("%s: Resetting Windows time quantum failed\n", __func__);
#endif
