...
#if defined(__AVR__)
typedef uint8_t atomicUnsignedInt_t;
#else
typedef unsigned int atomicUnsignedInt_t;
#endif
...
struct tsq_queue_t
{
    /** The ring buffer elements of the queue.\n
        ...
        other members \a idxRead and \a idxWrite. */
    void *ringBuffer;
...
#if TSQ_ENABLE_API_QUEUE_DIAGNOSTICS == 1
    /** The maximum number of elements, which has been queued so far. This number can be
        used at application validation time for optimization of the required queue size.\n
          This member is updated only by the producer code and is not subject to race
        conditions. */
    ...
    /** The position of the current tail of the list of data elements currently in the ring
        buffer. The name of this member results from the idea of using the ring buffer to
        implement a queue; new elements are appended to the tail of the queue and elder,
        consumed ones are removed from the (stale) head.\n
...
static inline void *allocTailElement(tsq_queue_t *pQueue, unsigned int *pNewIdxTail)
{
    ...
    /// @todo Loop could be replaced by the direct computation of the cyclic difference
...
static inline void postTailElement(tsq_queue_t *pQueue, unsigned int newIdxTail)
{
    ...
    /* Update the count of the free elements. The assertion catches forbidden underflows. */
    -- pQueue->noFreeElements;
    TSQ_ASSERT(pQueue->noFreeElements < pQueue->lengthOfRingBuffer);
 ...
 *   @see const void *tsq_readFromHead(tsq_queue_t *)
 */

bool tsq_writeToTail(tsq_queue_t *pQueue, const void *pData, unsigned int noBytes)
{
    /* No action (no partial data copying) if the written chunk doesn't fit. */
...
const void *tsq_readFromHead(tsq_queue_t *pQueue)
{
 ...
 * the use profile of the queue. No essential decision must be based on the result.
 * (Emitting a warning at high watermark could be appropriate, while raising an error is
 * rather not.) The most meaningful result is gained if the method is called from the
 * producer context and immediately after appending an element to the queue.
 *   @return
 * Get the number of currently queued elements.
 *   @param pQueue
 * The queue object by reference.
...
unsigned int tsq_getNoQueuedElements(tsq_queue_t *pQueue)
{
    /* If atomicUnsignedInt_t is shorter than unsigned int then pQueue->noFreeElements can
       be read safely only from the producer context. */
