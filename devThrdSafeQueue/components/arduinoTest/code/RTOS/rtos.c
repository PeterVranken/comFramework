 ...
 *   rtos_enableIRQTimerTick (callback with local default implementation)
 *   rtos_enableIRQUser00 (callback without default implementation)
 *   rtos_enableIRQUser00 (callback without default implementation)
 *   ISR(RTOS_ISR_SYSTEM_TIMER_TICK)
...
RTOS_DEFAULT_FCT void rtos_enableIRQTimerTick(void);
static RTOS_TRUE_FCT boolean onTimerTick(void);
...

{
    ...
    /** @todo It is also possible to do specific initialization of any other available timer
        ...
        of the interrupt vector in use. Modify #RTOS_ISR_SYSTEM_TIMER_TICK to do so. */
    TIMSK2 |= _BV(TOIE2);
#else
# error Modification of code for other AVR CPU required
#endif
...
static RTOS_TRUE_FCT boolean onTimerTick(void)
{
    ...
    while(idxSuspTask<_noSuspendedTasks)
    {
        ...
        /* Remember the received events before (possibly) getting some more in this
           timer tics: Only if this set changes it is necessary to check for a state
 ...
 *   @see boolean onTimerTick(void)
 *   @see #rtos_enterCriticalSection
 */

ISR(RTOS_ISR_SYSTEM_TIMER_TICK, ISR_NAKED)
{
    /* An ISR must not occur while we're updating the global data and checking for a
       possible task switch. To be more precise: The call of onTimerTick would just require
    ...
    if(onTimerTick())
    {
        /* Yes, another task becomes active with this timer tick. Switch the stack pointer
    ...
    /* The highly critical operation of modifying the stack pointer is done. From now on,
       ...
       ticks less of responsiveness of the system). */
    ...
    if((eventMask & RTOS_EVT_ABSOLUTE_TIMER) != 0)
    {
        ...
        if((intTime_t)((pT->timeDueAt+=timeout) - _time) <= 0)
        {
            ...
            /* The wanted point in time is over. We do the best recovery which is possible: Let
               the task become due in the very next timer tick.
                                         ...
                                         )
{
    /* It is absolutely essential that this routine is implemented as naked and noinline.
       ...
       calling parameters of the function are stored in such a stack frame. In the
       ...
       does save the local parameters into the (not existing) stack frame as very first
 ...
 * range is 1..max_value(\a uintTime_t). The effective length of the time slice is less
