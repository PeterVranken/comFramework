...
#define RTOS_RTUINOS_STARTUP_MSG                                                        \
    "RTuinOS " RTOS_RTUINOS_VERSION " for Arduino 1.0.6\n"                              \
    "Copyright (C) 2012-2016 Peter Vranken (mailto:Peter_Vranken@Yahoo.de)\n"           \
...
/** The system timer frequency as floating point constant. The unit is Hz.\n
      The value is derived from  #RTOS_TICK, which is about 2 ms in the RTuinOS standard
...
#define RTOS_TICK_FREQUENCY (1.0/(RTOS_TICK))

/** The scale factor between RTuinOS' system timer tick and Arduinos \a millis() as a
    floating point constant. Same as tick period in unit ms.
      The value is derived from  #RTOS_TICK, which is about 2 ms in the RTuinOS standard
    configuration. The macro is defined in the configuration file rtos.config.h as it might
    be subject to changes by the application. */
#define RTOS_TICK_MS ((RTOS_TICK)*1000.0)
 ...
 * 0..max(uintTime_t). The resolution of any timing operation is the tick of the system
