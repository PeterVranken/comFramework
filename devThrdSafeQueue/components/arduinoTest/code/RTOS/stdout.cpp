 ...
 *   @param baudRateOfSerial
 * The stream Serial is initialized by this function. To do so, we need to know at which
 * Baud rate it is run.
 */

extern "C" void init_stdout(uint16_t baudRateOfSerial)
{
    /* Start serial port at wanted transmission speed. */
    Serial.begin(baudRateOfSerial);
...
extern "C" int puts_progmem(const char *string)
{
    while(true)
    {
