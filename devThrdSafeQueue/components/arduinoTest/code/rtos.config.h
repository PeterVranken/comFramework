...
/** Select the interrupt which clocks the system time. Side effects to consider: This
    ...
    rtos_enableIRQTimerTick(void), which is an overridable default implementation.\n
      If an application redefines the interrupt source, the new source will probably
    produce another system clock frequency. If so, the macro #RTOS_TICK needs to be redefined
    also. */
#define RTOS_ISR_SYSTEM_TIMER_TICK TIMER2_OVF_vect


/** The system timer tick is about 2 ms. For more accurate considerations, it is defined here as
    floating point constant. The unit is s. */
#define RTOS_TICK (2.04e-3)
 ...
 *   @see #RTOS_ISR_SYSTEM_TIMER_TICK
...
/** The type of the system time. The system time is a cyclic integer value. If the use case
    ...
    waiting for events. The resolution is the tick frequency of the system time. With an 8
    Bit system time one would probably choose a tick frequency identical to the repetition
    ...
    it will end e.g. 87 ticks later, i.e. at 210. The demanded resume time is 123+100 = 223,
    which is seen as +13 in the future. If the task execution was too long and ended e.g.
    after 110 ticks, the system time was 123+110 = 233. The demanded resume time 223 is seen
    in the past and a task overrun is recognized. A problem appears at excessive task
    overruns. If the execution had e.g. taken 230 ticks the current time is 123 + 230 = 353 -
    or 97 due to its cyclic character. The demanded resume time 223 is 126 ticks ahead,
    which is considered a future time - no task overrun is recognized. The problem appears
    if the overrun lasts more than half the system timer cycle. With uint16_t this problem
    becomes negligible.\n
...
/** Normally, when the overrun of a regular task has been recognized the task is made due
    ...
    time greater than half the timer cycle (i.e. above 127 ticks) the probability of faulty
