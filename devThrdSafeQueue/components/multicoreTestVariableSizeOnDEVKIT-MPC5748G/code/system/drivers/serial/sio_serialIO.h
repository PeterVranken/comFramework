...
/** Efficient output of literal strings, without going through the C lib: The string is
    directly passed to sio_osWriteSerial.
      @param textWithoutLF
    The printed text as a C string literal, i.e. enclosed in double quotes. String
    expressions, e.g. a pointer to char are not supported by the macro. As the C lib
    function, the macro adds a newline to the printed text.
      @remark
    The macro must be used by supervisor contexts only, i.e. ISRs and OS tasks. It can be
    called from any core. */
#define SIO_osPUTS(textWithoutLF)   \
            (sio_osWriteSerial(textWithoutLF"\r\n",sizeof(textWithoutLF"\r\n")-1))
...
/** Efficient output of literal strings, without going through the C lib: The string is
    ...
    function, the macro adds a newline to the printed text.
      @remark
    The macro must be used by user code contexts only, i.e. user tasks. It can be called
    from any core. */
#define SIO_PUTS(textWithoutLF)     \
            (sio_writeSerial(textWithoutLF"\r\n",sizeof(textWithoutLF"\r\n")-1))
