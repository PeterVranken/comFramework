...
icn_errorCode_t icn_osInitInterCoreNotificationDriver(void)
{
    ...
    /* An array of function pointers to all SW settable interrupt handlers, which we
       required to implement the notifications. Note, the elements of the initializer
    ...
    for(idxN=0; idxN<ICN_NO_NOTIFICATIONS; ++idxN)
    {
        ...
        /* The vector number is validated by compile-time assertions, see above. */
 ...
 *   Race condition free implementation would be possible with respect to different contexts
