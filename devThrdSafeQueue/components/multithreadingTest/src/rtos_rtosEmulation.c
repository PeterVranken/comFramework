...
#if 0
    /* Improve test quality: Add a epsilon to the nominal time to avoid an all-time fixed
       phase shift between the tasks. */
    static long phaseShiftNs = 10000;
    tiCycleNs += phaseShiftNs;
    if(tiCycleNs > 1000000000)
    {
        tiCycleNs -= 1000000000;
        tiCycleS += 1;
    }
    phaseShiftNs += 7253;
#endif
...
#undef MIN_TI_CYCLE_TIMER_TASK
        ...
        while(!_endOfScheduling)
        {
            ...
            if(tiTimerTask >= tiDisplay)
            {
                ...
                printf( "Cycles: tst_noWrittenElements: %lu, tst_noEventsQueueFull: %lu,"
                        " tst_noReadElements: %lu\n"
                      , tst_noWrittenElements
